...
public class CalendarPickerView extends ListView implements NestedScrollingChild {
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        a.recycle();
       //viewModel = new ViewModelProvider(context).get(SharedViewModel.class);
        ...
        if (isInEditMode()) {
            ...
            init(new Date(), nextYear.getTime(), context) //
     ...
     * @param maxDate Latest selectable date, exclusive.  Must be later than {@code minDate}.
     */
    public FluentInitializer init(Date minDate, Date maxDate, Context context) {
        this.context = context;
    ...
    public class FluentInitializer {
        ...
        public void settSelectedDates(Collection<Date> selectedDates) {
            if (selectionMode == SelectionMode.SINGLE && selectedDates.size() > 1) {
                throw new IllegalArgumentException("SINGLE mode can't be used with multiple selectedDates");
            }
            if (selectionMode == SelectionMode.RANGE && selectedDates.size() > 2) {
        ...
        public void handleClick(MonthCellDescriptor cell) {
            ...
            if (!betweenDates(clickedDate, minCal, maxCal) || !isDateSelectable(clickedDate)) {
                ...
                if (dateListener != null) {
                    if (wasSelected) {
                        dateListener.onDateSelected(selectedCells);
                    } else {
                        dateListener.onDateUnselected(selectedCells);
                    }
                }
            }
        }
            ...
            for (int c = 0; c < 7; c++) {
                ...
                weekCells.add(
                    new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday,
                        isHighlighted, value, rangeState, month));
                cal.add(DATE, 1);
            }
        }
        return cells;
    ...
    public interface OnDateSelectedListener {
        ...
        void onDateSelected(List<MonthCellDescriptor> date);

        void onDateUnselected(List<MonthCellDescriptor> date);
    }
