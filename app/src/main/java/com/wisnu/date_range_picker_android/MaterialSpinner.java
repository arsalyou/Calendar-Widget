...
public class MaterialSpinner extends androidx.appcompat.widget.AppCompatTextView {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    if (!hideArrow) {
      arrowDrawable = Utils.getDrawable(context, R.drawable.ic_down).mutate();
...
//        }
    ...
    if (savedState instanceof Bundle) {
      ...
      if (adapter != null) {
        if (nothingSelected && !TextUtils.isEmpty(hintText)) {
          ...
          setTextColor(textColor);
          setText("Year " +adapter.get(selectedIndex).toString() + " ");
        }
  ...
  public void setSelectedIndex(int position) {
    if (adapter != null) {
      if (position >= 0 && position <= adapter.getCount()) {
        adapter.notifyItemSelected(position);
        selectedIndex = position;
        setText("Year " +adapter.get(position).toString()  + " ");
      } else {
  ...
  private void setAdapterInternal(@NonNull MaterialSpinnerBaseAdapter adapter) {
    ...
    setHintColor(R.color.colorPrimaryDark);
  }
